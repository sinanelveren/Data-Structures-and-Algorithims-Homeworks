 ...
 * constant.
 * @author Koffman and Wolfgang and Sinan Elveren - Gebze Technical University
...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    private static class Node<E> {
         ...
         *  the subtree of children smaller data[i] for i smaller size
         *  and to the subtree of children greater data[size-1]
         *  for i == size  */
     ...
     * @param order - the size of a node
     * @throws IllegalArgumentException if order smaller 3
     */
    ...
    private int binarySearch(E[] items, E target, int first, int last) {
        ...
        if (first > last)
            ...
            if (compResult == 0)
                return middle; // Base case for successfUl search  -- same element,so dont add it
            else if (compResult < 0) {
                if(middle < 1)
                    return middle;
                else if (target.compareTo(items[middle - 1]) > 0)
                    return middle;
                else
     ...
     *  node.data[index-1] &lt; item %lt; node.data[index];
     *  node.data[index] == item and old values are moved
     *       right one
