...
import com.SinanElverenHW6.RedBlackTree.BinarySearchTreeWithRotate;
import com.SinanElverenHW6.AVLTreeFromBinary.BinaryTree;
import com.SinanElverenHW6.*;
...
public class AVLTree < E
    ...
    public AVLTree(BinaryTree<E> tree) {
        ...
        if(isAVL(tree.root))
            System.out.println("Binary tree is AVL Tree\n\n");
        else {
            throw new IllegalArgumentException("Binary tree is NOT AVL Tree\n\n");
        }
    ...
    /**
     * Check binary tree, return true if it is avl tree
     * @param node tree node : root
     * @return true if avl tree, else false
     */
    protected boolean isAVL(BinaryTree.Node node){
        ...
        //add to avl tree node by node
        add((E) node.data);
        addReturn = false;
        increase = false;
        ((AVLNode<E>)this.root).balance = 0;
        ...
        if (Math.abs(leftHeight - rightHeight) <= 1 && isAVL(node.left) && isAVL(node.right))
    ...
    private int level(BinaryTree.Node<E> node) {
    ...
    /** Find the node such that parent.right.right == null
      The found node is removed from the tree and replaced
    ...
    /** Method to increment the balance field and to reset the value of
     ...
     from the left.
      The balance is incremented and the increase and decrease
    ...
    /** rebalanceRight
     ...
     more than one right heavy.
      balance is restored and increase is set false
